...
class SplitWorld : JavaPlugin(), Listener {
  ...
  private var worldConfigs: Map<String, WorldConfig> = config
    .getList("world_configs")!!
    .stream()
    .map { item: Any? -> WorldConfig((item as Map<String, Any>)) }
    .collect(
      Collectors.toMap({ item: WorldConfig -> item.worldName }, { item: WorldConfig -> item })
    )
  ...
  fun onPlayerMove(event: PlayerMoveEvent) {
    ...
    if (player.gameMode != GameMode.SURVIVAL && utils.locationOnSurvivalSide(event.to) && utils.locationOnSurvivalSide(
        player.location
      )
    ) {
      ...
      val playerHasElytraEquipped =
        player.inventory.chestplate != null && player.inventory.chestplate!!.type == Material.ELYTRA
  ...
  fun onPlayerRespawn(event: PlayerRespawnEvent) {
    ...
    if (customRespawn && !event.isAnchorSpawn && !event.isBedSpawn) {
      event.respawnLocation =
        Location(event.respawnLocation.world, coordinates[0], coordinates[1], coordinates[2], -88f, 6f)
    }
  ...
  fun onCreatureSpawn(event: CreatureSpawnEvent) {
    ...
    if (!utils.locationOnSurvivalSide(event.location) && event.spawnReason == CreatureSpawnEvent.SpawnReason.NATURAL && utils.getWorldConfig(
        world
      ).noCreativeMonsters && event.entity is Monster
    ) {
      event.isCancelled = true
    }
  }
